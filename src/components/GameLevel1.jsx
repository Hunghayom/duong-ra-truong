...
// Nhận 2 props âm lượng từ App.jsx
export default function GameLevel1({ onLevelComplete, bgmVolume, sfxVolume }) {
    ...
    const playSound = (soundName) => {
        try {
            ...
            audio.volume = sfxVolume; // Sử dụng âm lượng SFX từ App
    ...
    // Tự động cập nhật mức âm lượng BGM mỗi khi kéo thanh trượt ở App.jsx
    useEffect(() => {
    ...
    // LOGIC ĐỨNG CHUNG Ô (Tránh lệch mũi tên)
    const isOverlap = stats.art === stats.tech;
    // Nếu trùng, lệch sang 2 bên 15px. Nếu đứng 1 mình, căn chính giữa 100%
        ...
        <div className="w-full h-screen overflow-hidden bg-white px-4 py-2 md:px-8 md:py-4 flex flex-col items-center justify-between relative" style={{ fontFamily: "'Playpen Sans', cursive" }}>
            ...
            <div className="w-full max-w-6xl flex justify-between items-center mb-2 mt-8 md:mt-2 z-10">
                ...
                <div className="flex flex-col items-end">
                    ...
                    <div className="w-48 h-5 border-[3px] border-black p-[2px]" style={{ borderRadius: '6px' }}><div className="h-full transition-all duration-300" style={{ width: `${(happiness / 10) * 100}%`, backgroundColor: happiness > 5 ? '#ec4899' : '#ef4444', borderRadius: '3px' }}></div></div>
                </div>
            </div>
            ...
            <div className="w-full max-w-6xl relative my-10">
                ...
                {/* Nhân vật Nghệ thuật - Bọc trong w-[40px] để mũi tên luôn chính giữa */}
                <div className="absolute bottom-full mb-1 w-[40px] flex flex-col justify-end items-center transition-all duration-700 ease-in-out" style={{ left: `${getPosition(stats.art)}%`, transform: `translateX(${artOffset})`, zIndex: currentTurn === 'Nghệ thuật' ? 20 : 10 }}>
                    {currentTurn === 'Nghệ thuật' && step === 'CHOOSING' && <div className="absolute bottom-full mb-1 text-2xl font-bold animate-bounce text-pink-600">▼</div>}
                ...
                <div className="absolute bottom-full mb-1 w-[40px] flex flex-col justify-end items-center transition-all duration-700 ease-in-out" style={{ left: `${getPosition(stats.tech)}%`, transform: `translateX(${techOffset})`, zIndex: currentTurn === 'Công nghệ' ? 20 : 10 }}>
                    {currentTurn === 'Công nghệ' && step === 'CHOOSING' && <div className="absolute bottom-full mb-1 text-2xl font-bold animate-bounce text-blue-600">▼</div>}
                    <svg width="40" height="45" viewBox="0 0 40 50" fill="none" stroke="black" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
            ...
            <div className="flex-1 w-full max-w-6xl border-[3px] border-black p-4 md:p-6 relative bg-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col" style={{ borderRadius: '8px 12px 10px 8px' }}>
                ...
                {step === 'CHOOSING' ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full flex-1">
                            {currentEvent.choices.map((choice) => (
                                <div key={choice.id} onClick={() => handleCardClick(choice)} className="bg-white border-[3px] border-black p-4 flex flex-col cursor-pointer hover:bg-gray-50 transition-transform hover:-translate-y-1 h-full" style={{ borderRadius: '8px 10px 8px 12px' }}>
                                    {/* Tiêu đề căn giữa, Nội dung căn đều (justify) */}
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-center py-4">
                        {burnoutAlert ? (
                            <div className="mb-4 p-3 border-[3px] border-red-500 bg-red-50 text-red-700 font-bold max-w-2xl text-lg rounded-md">⚠️ KIỆT SỨC! Bạn đã vắt kiệt hạnh phúc. Cả 2 chỉ số bị lùi 3 ô!</div>
