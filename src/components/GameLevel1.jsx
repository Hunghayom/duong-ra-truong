...
export default function GameLevel1({ onLevelComplete }) {
  ...
  useEffect(() => {
    ...
    const handleEnded = () => {
      ...
      timeoutRef.current = setTimeout(playBGM, randomDelay);
    };
  ...
  // Cập nhật âm lượng nhạc nền khi kéo thanh Volume
  useEffect(() => {
    if (bgmRef.current) {
      bgmRef.current.volume = isMuted ? 0 : volume * 0.4; // Nhạc nền nhỏ hơn SFX 1 chút
    }
  }, [volume, isMuted]);
  ...
  // Tính toán Tọa độ & Trạng thái Đứng chung ô
  const trackStart = -5;
  const trackEnd = 20;
  ...
  // KIỂM TRA ĐỨNG CHUNG: Nếu 2 nhân vật cùng điểm, lệch mỗi người một chút ra 2 bên
  const isOverlap = stats.art === stats.tech;
  const artOffset = isOverlap ? '-25px' : '-50%';
  const techOffset = isOverlap ? '5px' : '-50%';
  ...
  return (
    <div className="w-full h-screen overflow-hidden bg-white px-4 py-2 md:px-8 md:py-4 flex flex-col items-center justify-between" style={{ fontFamily: "'Playpen Sans', cursive" }}>
      ...
      {/* KHUNG ĐIỀU KHIỂN ÂM THANH (Góc trên trái) */}
      <div className="absolute top-4 left-4 flex items-center gap-2 bg-white border-[2px] border-black px-3 py-1 z-50" style={{ borderRadius: '8px 10px 8px 12px' }}>
        <button onClick={() => setIsMuted(!isMuted)} className="hover:scale-110 transition-transform">
          <SpeakerIcon isMuted={isMuted} />
        </button>
        <input 
          type="range" min="0" max="1" step="0.05" 
          value={volume} onChange={(e) => { setVolume(parseFloat(e.target.value)); setIsMuted(false); }}
      ...
      <div className="w-full max-w-6xl relative my-10">
        ...
        {/* Nhân vật Nghệ thuật */}
        <div className="absolute bottom-full mb-1 transition-all duration-700 ease-in-out" style={{ left: `${getPosition(stats.art)}%`, transform: `translateX(${artOffset})`, zIndex: currentTurn === 'Nghệ thuật' ? 20 : 10 }}>
        ...
        {/* Nhân vật Công nghệ */}
        <div className="absolute bottom-full mb-1 transition-all duration-700 ease-in-out" style={{ left: `${getPosition(stats.tech)}%`, transform: `translateX(${techOffset})`, zIndex: currentTurn === 'Công nghệ' ? 20 : 10 }}>
          ...
          {[...Array(trackCells)].map((_, i) => {
            ...
            return (
              ...
              `}>
                ...
                {/* Thay chữ thành số 0 cho mốc xuất phát */}
                {isStart && <span className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-sm font-bold text-gray-800">0</span>}
                {isEnd && <span className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-xs font-bold text-red-600 whitespace-nowrap">KẾT THÚC NĂM HỌC</span>}
      ...
      <div className="flex-1 w-full max-w-6xl border-[3px] border-black p-4 md:p-6 relative bg-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col" style={{ borderRadius: '8px 12px 10px 8px' }}>
        ...
        {step === 'CHOOSING' ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center py-4">
            ...
            {/* THÔNG BÁO TĂNG GIẢM CẢ 3 CHỈ SỐ */}
            <div className="flex flex-wrap justify-center gap-6 mb-6 text-xl font-bold bg-gray-50 px-6 py-3 border-[3px] border-black" style={{ borderRadius: '8px' }}>
              ...
              {/* Logic tính Hạnh phúc cho màn hình hiển thị */}
              {(() => {
                 const hChange = (selectedChoice.tech < 0 || selectedChoice.art < 0) ? -2 : ((selectedChoice.tech >= 0 && selectedChoice.art >= 0) ? 1 : 0);
                 return (
                   <span className={hChange > 0 ? 'text-pink-500' : hChange < 0 ? 'text-red-500' : 'text-gray-500'}>
                     Hạnh phúc {hChange > 0 ? '+' : ''}{hChange}
                   </span>
                 );
              })()}
            </div>
