...
export default function SlideLayout({ title, text, isTitleSlide, children, onNextSlide, onSkipIntro }) {
    ...
    return (
        <div
            className="relative z-10 w-full h-full flex flex-col items-center justify-end pb-12 cursor-pointer overflow-hidden bg-transparent"
            ...
            >
                <div className="text-xl md:text-2xl leading-relaxed text-black font-semibold text-center w-full px-4">
                    ...
                    {currentLineIndex < lines.length && (
                        <motion.p
                            key={currentLineIndex} // Quan trọng: Key giúp React không làm nháy khung
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                            className="m-0"
                        >
